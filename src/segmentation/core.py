    ...
    def Segment(self, image: np.ndarray) -> List[np.ndarray]:
        ...
        for start, end in zip(start_indices, end_indices):
            if end - start > self.min_height:
                segment_crop = image[
                    max(0, start - self.margin) : min(
                        image.shape[0], end + self.margin
                    ),
                    :,
                ]
        ...
        for i, val in enumerate(vpp):
            if val > 0 and not in_segments:
                ...
                if i - segment_start > self.min_width:
                    segment_crop = image[
                        :,
                        max(0, segment_start - self.margin) : min(
                            image.shape[1], i + self.margin
                        ),
                    ]
                    segments.append(segment_crop)
                in_segments = False
        ...
        if in_segments:
            if len(vpp) - segment_start > self.min_width:
                segment_crop = image[
                    :,
                    max(0, segment_start - self.margin) : min(
                        image.shape[1], len(vpp) + self.margin
                    ),
                ]
                segments.append(segment_crop)
    ...
    def Segment(self, image: np.ndarray) -> List[np.ndarray]:
        ...
        for cnt in contours:
            ...
            if h >= self.min_height and w >= self.min_width:
                segment_crop = image[
                    max(0, y - self.margin) : min(image.shape[0], y + h + self.margin),
